...
with ipatch('amqp'):
    from gofer.messaging.adapter.amqp.producer import build_message, send, plain_send
    from gofer.messaging.adapter.amqp.producer import Producer, BaseProducer
    from gofer.messaging.adapter.amqp.producer import PlainProducer, BasePlainProducer
...
class TestSend(TestCase):
    ...
    @patch('gofer.messaging.adapter.amqp.producer.uuid4')
    @patch('gofer.messaging.adapter.amqp.producer.Message')
    @patch('gofer.messaging.adapter.amqp.producer.auth')
...
class TestProducer(TestCase):
    ...
    @patch('gofer.messaging.adapter.amqp.producer.send')
...
class TestPlainProducer(TestCase):
    ...
    @patch('gofer.messaging.adapter.amqp.producer.plain_send')
    def test_send(self, send):
        ttl = 10
        sn = 1234
        send.return_value = sn
